...
public class RedPacketReceiveService extends ServiceImpl<RedPacketMapper, RedPacket> {
    ...
    private static final DefaultRedisScript<Long> RED_PACKET_SCRIPT = new DefaultRedisScript<>(
            RedPacketConstants.RED_PACKET_LUA_SCRIPT.getValue(),
            Long.class
    );
    ...
    private Integer grabRedPacket(Long redPacketId) {
        ...
        try {
            Long result = redisTemplate.execute(RED_PACKET_SCRIPT, Collections.singletonList(redPacketCountKey));
            if (result == null) {
