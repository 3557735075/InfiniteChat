...
public class MessageServiceImpl extends ServiceImpl<MessageMapper, Message> implements MessageService {
    ...
    private static final String DEFAULT_SESSION_AVATAR = System.getenv().getOrDefault(
            "GROUP_AVATAR", "http://127.0.0.1:9000/infinitec-chat/IM_GROUP.jpg");
    ...
    private Cache<String, String> userRouteCache;
    ...
    public MessageServiceImpl(UserService userService,
                              ...
                              DiscoveryClient discoveryClient, RedisTemplate<String, String> redisTemplate, KafkaTemplate<String, String> kafkaTemplate) {
    ...
    public SendMsgResponse sendMessage(SendMsgRequest request) {
        ...
        // TODO: 发送到kafka
        sendKafkaMessage(request,messageId,createdAt);
        // 4.通过redis查询接收者的netty服务在哪
        sendRealTimeMessage(request, appMessage, createdAt);

        return buildResponseMsgVo(appMessage);
    }
    private void sendKafkaMessage(SendMsgRequest sendMsgRequest,  Long messageId, Date createdAt) {
    ...
    private void sendRealTimeMessage(SendMsgRequest sendMsgRequest, AppMessage appMessage, Date createdAt) {
        ...
        if (sendMsgRequest.getSessionType() == SessionType.SINGLE.getValue()) {
            ...
            sendGroupMessage(instances, requestBody);
        }
    }
    ...
    private void sendGroupMessage(List<ServiceInstance> instances, RequestBody requestBody) {
        for (ServiceInstance instance : instances) {
            groupMessageExecutor.submit(() -> {
                String url = instance.getUri().toString();
                Request request = new Request.Builder()
                        .url("http://"+url + ":8083"+ConfigEnum.MSG_URL.getValue())
                        .post(requestBody)
                        .build();
                try {
                    executeHttpRequest(request);
